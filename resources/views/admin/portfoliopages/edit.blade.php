...
<div class="card card-custom gutter-b">
    ...
    <div class="card-header">
        <div class="card-title">
            <h3 class="card-label">
                ...
                <a href="{{ route('admin.portfoliopages.edit') }}?lang={{ $locale }}" data-toggle="tooltip" data-theme="dark" title="{{ strtoupper($locale) }}" class="btn btn-icon {{ $locale == $lang ? 'btn-light-secondary' : '' }} btn-hover-secondary">
                    <img src="{{ asset('uploads/settings/languages/' . mb_strtolower($locale, 'UTF-8') . '.png') }}" class="h-30px w-30px mb-1 {{ (!$portfoliopage->hasTranslation($locale)) ? 'opacity-20' : '' }}" alt="{{ strtoupper($locale) }}" />
                </a>
                @endforeach  
                @endif
            </h3>
        ...
        <div class="card-toolbar">
            @can('portfoliopage_delete')
            @if($portfoliopage->hasTranslation($lang) && $lang != config('translatable.locale'))
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body text-left">
                            {!! trans('global.confirm_delete_question', ['resource' => $portfoliopage->name]) !!}
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary font-weight-bold float-right" data-dismiss="modal">{{ trans('global.close') }}</button>
                            <form action="{{ route('admin.portfoliopage.delete', $portfoliopage->id) }}" class="float-left" method="POST">
    ...
    <!--end::Header-->
    <form method="POST" action="{{ route("admin.portfoliopages.update", [$portfoliopage->id]) }}" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <input type="hidden" name="lang" value="{{ $lang }}">
        <!--begin::Body-->
        <div class="card-body pb-5 row">
            <div class="col-lg-8">
                <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="50" type="text" name="name" value="{{ old('name', $portfoliopage->translateOrDefault($lang)->name) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.name') }}">
                    @if($errors->has('name'))
                ...
                <div class="form-group {{ $errors->has('text') ? 'has-error' : '' }}">
                    ...
                    <textarea class="form-control form-control-solid tynimce" maxlength="100000" name="text" rows="3" placeholder="{{ trans('global.enter') }} {{ trans('cruds.text') }}">{{ old('text', $portfoliopage->translateOrDefault($lang)->text) }}</textarea>
                    @if($errors->has('text'))
                    <span class="help-block" role="alert">
            ...
            <div class="col-lg-12">
                ...
                <div class="form-group {{ $errors->has('slug') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="255" type="text" name="slug" value="{{ old('slug', $portfoliopage->translateOrDefault($lang)->slug) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.slug') }}">
                    @if($errors->has('slug'))
                    <span class="help-block" role="alert">
                ...
                <div class="form-group {{ $errors->has('seotitle') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="255" type="text" name="seotitle" value="{{ old('seotitle', $portfoliopage->translateOrDefault($lang)->seotitle) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.seotitle') }}">
                ...
                <div class="form-group {{ $errors->has('seodescription') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="255" type="text" name="seodescription" value="{{ old('seodescription', $portfoliopage->translateOrDefault($lang)->seodescription) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.seodescription') }}">
...
<div class="form-group {{ $errors->has('seoimage') ? 'has-error' : '' }}">
                ...
                <div class="form-group {{ $errors->has('seotype') ? 'has-error' : '' }}">
                    ...
                    <select class="form-control select2 select2_dropdown" name="seotype">
                        ...
                        <option value="{{ $k }}" {{ old('seotype', $portfoliopage->translateOrDefault($lang)->seotype) == $k ? "selected" : "" }}>
                            {{ $v }}
                        </option>
                        @endforeach
                    </select>
...
<script>
    ...
    Dropzone.options.seoimageDropzone = {
        url: '<?= route('admin.portfoliopage.storeMedia') ?>',
        ...
        init: function () {
<?php if (isset($portfoliopage) && $portfoliopage->seoimage) : ?>
                var file = <?= json_encode($portfoliopage->seoimage) ?>;
