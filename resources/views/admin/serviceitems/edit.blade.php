...
<div class="card card-custom gutter-b">
    ...
    <div class="card-header">
        <div class="card-title">
            <h3 class="card-label">
                ...
                <a href="{{ route('admin.serviceitems.edit', $serviceitem->id) }}?lang={{ $locale }}" data-toggle="tooltip" data-theme="dark" title="{{ strtoupper($locale) }}" class="btn btn-icon {{ $locale == $lang ? 'btn-light-secondary' : '' }} btn-hover-secondary">
                    <img src="{{ asset('uploads/settings/languages/' . mb_strtolower($locale, 'UTF-8') . '.png') }}" class="h-30px w-30px mb-1 {{ (!$serviceitem->hasTranslation($locale)) ? 'opacity-20' : '' }}" alt="{{ strtoupper($locale) }}" />
                </a>
                @endforeach  
                @endif
            </h3>
        ...
        <div class="card-toolbar">
            @can('serviceitem_delete')
            @if($serviceitem->hasTranslation($lang) && $lang != config('translatable.locale'))
            <!-- Button trigger modal-->
            @if(config('cms.submenu_only_icons') == true)
            <button type="button" class="btn btn-light-danger font-weight-bold btn-hover-danger btn-sm btn-icon d-sm-none" data-toggle="modal" data-target="#delete{{ $serviceitem->id }}">
                <i class="text-danger far fa-trash-alt"></i>
            </button>
            <button type="button" class="btn btn-light-danger font-weight-bold btn-hover-danger btn-sm d-none d-sm-block" data-toggle="modal" data-target="#delete{{ $serviceitem->id }}">
                <i class="text-danger far fa-trash-alt"></i>
                {{ trans('global.delete') }}
            </button>
            @else
            <button type="button" class="btn btn-light-danger font-weight-bold btn-hover-danger btn-sm" data-toggle="modal" data-target="#delete{{ $serviceitem->id }}">
                <i class="text-danger far fa-trash-alt"></i>
                {{ trans('global.delete') }}
            </button>
            @endif
            <!-- Modal-->
            <div class="modal fade" id="delete{{ $serviceitem->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body text-left">
                            {!! trans('global.confirm_delete_question', ['resource' => $serviceitem->name]) !!}
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary font-weight-bold float-right" data-dismiss="modal">{{ trans('global.close') }}</button>
                            <form action="{{ route('admin.serviceitem.delete', $serviceitem->id) }}" class="float-left" method="POST">
    ...
    <!--end::Header-->
    <form method="POST" action="{{ route("admin.serviceitems.update", [$serviceitem->id]) }}" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <input type="hidden" name="lang" value="{{ $lang }}">
        <!--begin::Body-->
        <div class="card-body pb-5 row">
            <div class="col-lg-8">
                <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="150" type="text" name="name" value="{{ old('name', $serviceitem->translateOrDefault($lang)->name) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.name') }}">
                    @if($errors->has('name'))
            ...
            <div class="col-lg-12">
                ...
                <div class="form-group {{ $errors->has('slug') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="255" type="text" name="slug" value="{{ old('slug', $serviceitem->translateOrDefault($lang)->slug) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.slug') }}">
                    @if($errors->has('slug'))
                    <span class="help-block" role="alert">
                ...
                <div class="form-group {{ $errors->has('seotitle') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="255" type="text" name="seotitle" value="{{ old('seotitle', $serviceitem->translateOrDefault($lang)->seotitle) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.seotitle') }}">
                ...
                <div class="form-group {{ $errors->has('seodescription') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="255" type="text" name="seodescription" value="{{ old('seodescription', $serviceitem->translateOrDefault($lang)->seodescription) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.seodescription') }}">
...
<div class="form-group {{ $errors->has('seoimage') ? 'has-error' : '' }}">
                ...
                <div class="form-group {{ $errors->has('seotype') ? 'has-error' : '' }}">
                    ...
                    <select class="form-control select2 select2_dropdown" name="seotype">
                        ...
                        <option value="{{ $k }}" {{ old('seotype', $serviceitem->translateOrDefault($lang)->seotype) == $k ? "selected" : "" }}>
                            {{ $v }}
                        </option>
                        @endforeach
                    </select>
