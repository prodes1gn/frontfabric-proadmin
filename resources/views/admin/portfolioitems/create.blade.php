    ...
    <div class="card-header">
        <div class="card-title">
            <h3 class="card-label">
                ...
                @endif
                {{ trans('cruds.portfolioitem') }}
                <small>{{ $task }}</small>
            </h3>
        </div>
    </div>
    <!--end::Header-->
    <form method="POST" action="{{ route("admin.portfolioitems.store") }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="card-body pb-5 row">
            ...
            <div class="col-lg-4">
                ...
                <div class="form-group {{ $errors->has('servicedropdown') ? 'has-error' : '' }}">
                    ...
                    <select class="form-control form-control-solid select2 select2_multiselect" name="servicedropdown[]" multiple>
                        @foreach($servicedropdowns as $k => $v)
                        <option value="{{ $k }}" {{ in_array($k, old('servicedropdown', [])) ? 'selected' : '' }}>{{ $v }}</option>
                        @endforeach
                    </select>
