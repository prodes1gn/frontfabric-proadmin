...
<div class="card card-custom gutter-b">
    ...
    <div class="card-header">
        <div class="card-title">
            <h3 class="card-label">
                ...
                <a href="{{ route('admin.portfolioitems.edit', $portfolioitem->id) }}?lang={{ $locale }}" data-toggle="tooltip" data-theme="dark" title="{{ strtoupper($locale) }}" class="btn btn-icon {{ $locale == $lang ? 'btn-light-secondary' : '' }} btn-hover-secondary">
                    <img src="{{ asset('uploads/settings/languages/' . mb_strtolower($locale, 'UTF-8') . '.png') }}" class="h-30px w-30px mb-1 {{ (!$portfolioitem->hasTranslation($locale)) ? 'opacity-20' : '' }}" alt="{{ strtoupper($locale) }}" />
                </a>
                @endforeach  
                @endif
            </h3>
        ...
        <div class="card-toolbar">
            @can('portfolioitem_delete')
            @if($portfolioitem->hasTranslation($lang) && $lang != config('translatable.locale'))
            <!-- Button trigger modal-->
            @if(config('cms.submenu_only_icons') == true)
            <button type="button" class="btn btn-light-danger font-weight-bold btn-hover-danger btn-sm btn-icon d-sm-none" data-toggle="modal" data-target="#delete{{ $portfolioitem->id }}">
                <i class="text-danger far fa-trash-alt"></i>
            </button>
            <button type="button" class="btn btn-light-danger font-weight-bold btn-hover-danger btn-sm d-none d-sm-block" data-toggle="modal" data-target="#delete{{ $portfolioitem->id }}">
                <i class="text-danger far fa-trash-alt"></i>
                {{ trans('global.delete') }}
            </button>
            @else
            <button type="button" class="btn btn-light-danger font-weight-bold btn-hover-danger btn-sm" data-toggle="modal" data-target="#delete{{ $portfolioitem->id }}">
                <i class="text-danger far fa-trash-alt"></i>
                {{ trans('global.delete') }}
            </button>
            @endif
            <!-- Modal-->
            <div class="modal fade" id="delete{{ $portfolioitem->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body text-left">
                            {!! trans('global.confirm_delete_question', ['resource' => $portfolioitem->name]) !!}
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary font-weight-bold float-right" data-dismiss="modal">{{ trans('global.close') }}</button>
                            <form action="{{ route('admin.portfolioitem.delete', $portfolioitem->id) }}" class="float-left" method="POST">
    ...
    <!--end::Header-->
    <form method="POST" action="{{ route("admin.portfolioitems.update", [$portfolioitem->id]) }}" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <input type="hidden" name="lang" value="{{ $lang }}">
        <!--begin::Body-->
        <div class="card-body pb-5 row">
            <div class="col-lg-8">
                <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="150" type="text" name="name" value="{{ old('name', $portfolioitem->translateOrDefault($lang)->name) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.name') }}">
                    @if($errors->has('name'))
            ...
            <div class="col-lg-4" style="{{ ($lang != config('translatable.locale')) ? "visibility:hidden" : ""  }}">
                ...
                <div class="form-group {{ $errors->has('servicedropdown') ? 'has-error' : '' }}">
                    <label for="servicedropdown">{{ trans('cruds.servicedropdown') }} <span class="required"></span></label>
                    <select class="form-control select2 select2_multiselect" name="servicedropdown[]" multiple>
                        @foreach($servicedropdowns as $k => $v)
                        <option value="{{ $k }}" {{ (in_array($k, old('servicedropdown', [])) || $portfolioitem->servicedropdown->contains($k)) ? 'selected' : '' }}>{{ $v }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('servicedropdown'))
                    <span class="help-block" role="alert">
                        @foreach($errors->get('servicedropdown') as $message)
                        {{ $message }}<br />
                        @endforeach
                    </span>
                    @endif
...
<div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
            ...
            <div class="col-lg-12">
                ...
                <div class="form-group {{ $errors->has('slug') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="255" type="text" name="slug" value="{{ old('slug', $portfolioitem->translateOrDefault($lang)->slug) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.slug') }}">
                    @if($errors->has('slug'))
                    <span class="help-block" role="alert">
                ...
                <div class="form-group {{ $errors->has('seotitle') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="255" type="text" name="seotitle" value="{{ old('seotitle', $portfolioitem->translateOrDefault($lang)->seotitle) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.seotitle') }}">
                ...
                <div class="form-group {{ $errors->has('seodescription') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="255" type="text" name="seodescription" value="{{ old('seodescription', $portfolioitem->translateOrDefault($lang)->seodescription) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.seodescription') }}">
...
<div class="form-group {{ $errors->has('seoimage') ? 'has-error' : '' }}">
                ...
                <div class="form-group {{ $errors->has('seotype') ? 'has-error' : '' }}">
                    ...
                    <select class="form-control select2 select2_dropdown" name="seotype">
                        ...
                        <option value="{{ $k }}" {{ old('seotype', $portfolioitem->translateOrDefault($lang)->seotype) == $k ? "selected" : "" }}>
                            {{ $v }}
                        </option>
                        @endforeach
                    </select>
        ...
        <div class="card-footer d-flex justify-content-between">
            <div class="form-group-show">
                <a type="submit" href="{{ route("admin.portfolioitems.index") }}" class="btn btn-secondary font-weight-bold">
...
<script>
    ...
    Dropzone.options.seoimageDropzone = {
        url: '<?= route('admin.portfolioitem.storeMedia') ?>',
        ...
        init: function () {
<?php if (isset($portfolioitem) && $portfolioitem->seoimage) : ?>
                var file = <?= json_encode($portfolioitem->seoimage) ?>;
...
<script>
    ...
    Dropzone.options.imageDropzone = {
        ...
        init: function () {
<?php if (isset($portfolioitem) && $portfolioitem->image) : ?>
                var file = <?= json_encode($portfolioitem->image) ?>;
                this.options.addedfile.call(this, file)
                this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
                file.previewElement.classList.add('dz-complete')
