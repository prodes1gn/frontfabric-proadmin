...
<div class="card card-custom gutter-b">
    ...
    <div class="card-header">
        <div class="card-title">
            <h3 class="card-label">
                ...
                <a href="{{ route('admin.contactspages.edit') }}?lang={{ $locale }}" data-toggle="tooltip" data-theme="dark" title="{{ strtoupper($locale) }}" class="btn btn-icon {{ $locale == $lang ? 'btn-light-secondary' : '' }} btn-hover-secondary">
                    <img src="{{ asset('uploads/settings/languages/' . mb_strtolower($locale, 'UTF-8') . '.png') }}" class="h-30px w-30px mb-1 {{ (!$contactspage->hasTranslation($locale)) ? 'opacity-20' : '' }}" alt="{{ strtoupper($locale) }}" />
                </a>
                @endforeach  
                @endif
            </h3>
        ...
        <div class="card-toolbar">
            @can('contactspage_delete')
            @if($contactspage->hasTranslation($lang) && $lang != config('translatable.locale'))
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body text-left">
                            {!! trans('global.confirm_delete_question', ['resource' => $contactspage->name]) !!}
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary font-weight-bold float-right" data-dismiss="modal">{{ trans('global.close') }}</button>
                            <form action="{{ route('admin.contactspage.delete', $contactspage->id) }}" class="float-left" method="POST">
    ...
    <!--end::Header-->
    <form method="POST" action="{{ route("admin.contactspages.update", [$contactspage->id]) }}" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <input type="hidden" name="lang" value="{{ $lang }}">
        <!--begin::Body-->
        <div class="card-body pb-5 row">
            <div class="col-lg-8">
                <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="50" type="text" name="name" value="{{ old('name', $contactspage->translateOrDefault($lang)->name) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.name') }}">
                    @if($errors->has('name'))
                ...
                <div class="form-group {{ $errors->has('text') ? 'has-error' : '' }}">
                    ...
                    <textarea class="form-control form-control-solid tynimce" maxlength="100000" name="text" rows="3" placeholder="{{ trans('global.enter') }} {{ trans('cruds.text') }}">{{ old('text', $contactspage->translateOrDefault($lang)->text) }}</textarea>
                    @if($errors->has('text'))
                    <span class="help-block" role="alert">
            ...
            <div class="col-lg-12">
                ...
                <div class="form-group {{ $errors->has('slug') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="255" type="text" name="slug" value="{{ old('slug', $contactspage->translateOrDefault($lang)->slug) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.slug') }}">
                    @if($errors->has('slug'))
                    <span class="help-block" role="alert">
                ...
                <div class="form-group {{ $errors->has('seotitle') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="255" type="text" name="seotitle" value="{{ old('seotitle', $contactspage->translateOrDefault($lang)->seotitle) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.seotitle') }}">
                ...
                <div class="form-group {{ $errors->has('seodescription') ? 'has-error' : '' }}">
                    ...
                    <input class="form-control form-control-solid maxlength" maxlength="255" type="text" name="seodescription" value="{{ old('seodescription', $contactspage->translateOrDefault($lang)->seodescription) }}" placeholder="{{ trans('global.enter') }} {{ trans('cruds.seodescription') }}">
...
<div class="form-group {{ $errors->has('seoimage') ? 'has-error' : '' }}">
                ...
                <div class="form-group {{ $errors->has('seotype') ? 'has-error' : '' }}">
                    ...
                    <select class="form-control select2 select2_dropdown" name="seotype">
                        ...
                        <option value="{{ $k }}" {{ old('seotype', $contactspage->translateOrDefault($lang)->seotype) == $k ? "selected" : "" }}>
                            {{ $v }}
                        </option>
                        @endforeach
                    </select>
...
<script>
    ...
    Dropzone.options.seoimageDropzone = {
        url: '<?= route('admin.contactspage.storeMedia') ?>',
        ...
        init: function () {
<?php if (isset($contactspage) && $contactspage->seoimage) : ?>
                var file = <?= json_encode($contactspage->seoimage) ?>;
