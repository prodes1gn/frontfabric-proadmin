...
<div>
    <div class="mb-10">
        <h3 class="opacity-85">{{ trans('global.register') }}</h3>
        <div class="font-weight-bold font-size-lg opacity-55">{{ trans('global.enter_your_details') }}</div>
    </div>
    ...
    <form method="POST" action="{{ route('register') }}" class="form">
        ...
        <div class="form-group mb-5 {{ $errors->has('name') ? 'has-error' : '' }}">
            <input type="text" name="name" class="form-control h-auto form-control-solid py-4 px-5 font-size-lg maxlength" maxlength="50" autofocus placeholder="{{ trans('global.user_name') }}" value="{{ old('name', null) }}">
            @if($errors->has('name'))
            <p class="help-block">
                {{ $errors->first('name') }}
            </p>
        ...
        <div class="form-group mb-5 {{ $errors->has('email') ? 'has-error' : '' }}">
            <input type="email" name="email" class="form-control h-auto form-control-solid py-4 px-5 font-size-lg maxlength" maxlength="150" placeholder="{{ trans('global.login_email') }}" value="{{ old('email', null) }}">
            @if($errors->has('email'))
            <p class="help-block">
                {{ $errors->first('email') }}
            </p>
