...
class User extends Authenticatable implements HasMedia {
    ...
    public function __construct(array $attributes = []) {
        ...
        self::created(function (User $user) {
            if (auth()->check()) {
                ...
                $registrationRole = config('cms.registration_default_role');
    ...
    public function generateTwoFactorCode() {
        ...
        $this->two_factor_expires_at = now()->addMinutes(15)->format(config('cms.date_format') . ' ' . config('cms.time_format'));
        $this->save();
    }
    ...
    public function getEmailVerifiedAtAttribute($value) {
        return $value ? Carbon::createFromFormat('Y-m-d H:i:s', $value)->format(config('cms.date_format') . ' ' . config('cms.time_format')) : null;
    }

    public function setEmailVerifiedAtAttribute($value) {
        $this->attributes['email_verified_at'] = $value ? Carbon::createFromFormat(config('cms.date_format') . ' ' . config('cms.time_format'), $value)->format('Y-m-d H:i:s') : null;
    }
    ...
    public function setTwoFactorExpiresAtAttribute($value) {
        $this->attributes['two_factor_expires_at'] = $value ? Carbon::createFromFormat(config('cms.date_format') . ' ' . config('cms.time_format'), $value)->format('Y-m-d H:i:s') : null;
    }
